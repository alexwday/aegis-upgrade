...
async def call_tool_prompt(
    ...
    response = await complete_with_tools(
        ...
        context=context,
        llm_params={
            "model": getattr(config.llm, PILLAR3_MODEL_TIER).model,
            "temperature": 0,
...
def load_stage_prompt(prompt_name: str, execution_id: Optional[str] = None) -> Dict[str, Any]:
    """Load a Pillar 3 stage prompt from DB, falling back to YAML."""
    logger = get_logger()
    try:
        ...
        logger.warning(
            "subagent.pillar3.prompt_db_fallback",
            execution_id=execution_id,
...
def load_stage_prompt_from_yaml(prompt_name: str) -> Dict[str, Any]:
    """Load a Pillar 3 stage prompt from package YAML."""
