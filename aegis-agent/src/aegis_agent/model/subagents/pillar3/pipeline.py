"""Staged retrieval pipeline for Pillar 3."""
...
DATA_TABLE = 'public."aegis-pillar3-data"'
EMBEDDINGS_TABLE = 'public."aegis-pillar3-embeddings"'
PILLAR3_MODEL_TIER = "small"
...
async def run_retrieval_pipeline(
    ...
    logger.info(
        "subagent.pillar3.pipeline_complete",
...
async def process_combo_retrieval(
    ...
    expanded = await gap_fill_one_sheet_gaps(
        reranked, search_semaphore=search_semaphore
    )
...
async def prepare_query(
    ...
    try:
        ...
        logger.warning(
            "subagent.pillar3.query_prep_fallback",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
...
async def embed_prepared_query(
    ...
    try:
        ...
        logger.warning(
            "subagent.pillar3.query_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        prepared["embeddings"] = {}
...
async def rerank_candidates(
    ...
    try:
        ...
        logger.warning(
            "subagent.pillar3.rerank_keep_all",
            execution_id=context.get("execution_id"),
    ...
    logger.info(
        "subagent.pillar3.rerank_complete",
        execution_id=context.get("execution_id"),
        candidate_count=len(candidates),
        ...
        sheet_numbers = sorted(
            {
                sheet_number
                for sheet_number in (
                    parse_sheet_number(chunk.get("chunk_id", ""))
...
async def run_research_loop(
    ...
    for iteration_number in range(1, RESEARCH_MAX_ITERATIONS + 1):
        try:
            ...
            logger.warning(
                "subagent.pillar3.research_iteration_failed",
                execution_id=context.get("execution_id"),
        ...
        chunks = await gap_fill_one_sheet_gaps(
...
async def search_additional_queries(
    ...
    try:
        ...
        logger.warning(
            "subagent.pillar3.additional_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        return []
...
async def call_tool_prompt(
    ...
    response = await complete_with_tools(
        ...
        context=context,
        llm_params={
            "model": resolve_research_model(context, PILLAR3_MODEL_TIER),
            "temperature": 0,
...
def combo_params(combo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "bank_symbol": normalize_pillar3_bank_symbol(bank_symbol),
        "fiscal_year": fiscal_year,
        "quarter": str(combo.get("quarter", "")).strip().upper(),
    }


def normalize_pillar3_bank_symbol(bank_symbol: Any) -> str:
    """Return the bank symbol exactly as stored in Pillar 3 metadata."""
...
def format_retrieval_response(results: Dict[str, Any]) -> str:
    ...
    for combo_result in results["combo_results"]:
        ...
        if findings:
            ...
            lines.append("- No Pillar 3 content was found for this bank/period.")
        else:
