"""Staged retrieval pipeline for investor slides."""
...
DATA_TABLE = 'public."aegis-investor-slides-data"'
EMBEDDINGS_TABLE = 'public."aegis-investor-slides-embeddings"'
PROMPTS_DIR = Path(__file__).resolve().parent / "prompts"
INVESTOR_SLIDES_MODEL_TIER = "small"
...
async def run_retrieval_pipeline(
    ...
    """Run query prep, hybrid search, rerank, page gap fill, and research."""
    logger = get_logger()
    ...
    logger.info(
        "subagent.investor_slides.pipeline_complete",
...
async def prepare_query(
    ...
    try:
        ...
        logger.warning(
            "subagent.investor_slides.query_prep_fallback",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
...
def fallback_prepared_query(query_text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "hyde_answer": soften_hyde_answer(
            "The source should disclose the requested slide metrics, period "
            "comparisons, and any relevant page callouts for the selected banks."
        ),
        "embeddings": {},
        "usage": {},
    }
...
async def embed_prepared_query(prepared: Dict[str, Any], context: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            "subagent.investor_slides.query_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        prepared["embeddings"] = {}
...
async def rerank_candidates(
    ...
    try:
        ...
        logger.warning(
            "subagent.investor_slides.rerank_keep_all",
            execution_id=context.get("execution_id"),
    ...
    logger.info(
        "subagent.investor_slides.rerank_complete",
        execution_id=context.get("execution_id"),
        candidate_count=len(candidates),
...
async def load_page_chunks(file_id: str, page_number: int) -> List[Dict[str, Any]]:
    """Load every chunk for one PDF page number."""
    query = text(
        f"""
        SELECT
            d.source_type,
...
async def run_research_loop(
    ...
    for iteration_number in range(1, RESEARCH_MAX_ITERATIONS + 1):
        try:
            ...
            logger.warning(
                "subagent.investor_slides.research_iteration_failed",
                execution_id=context.get("execution_id"),
...
async def call_research_iteration(
    ...
    replacements = {
        ...
        "source_label": "Investor slides package",
...
async def search_additional_queries(
    ...
    try:
        ...
        logger.warning(
            "subagent.investor_slides.additional_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        return []
...
async def call_tool_prompt(
    ...
    response = await complete_with_tools(
        ...
        context=context,
        llm_params={
            "model": getattr(config.llm, INVESTOR_SLIDES_MODEL_TIER).model,
            "temperature": 0,
...
def load_stage_prompt(prompt_name: str, execution_id: Optional[str] = None) -> Dict[str, Any]:
    """Load an investor slides stage prompt from DB, falling back to YAML."""
    logger = get_logger()
    try:
        prompt_data = load_prompt_from_db(
            layer="investor_slides",
            name=prompt_name,
        ...
        logger.warning(
            "subagent.investor_slides.prompt_db_fallback",
            execution_id=execution_id,
...
def load_stage_prompt_from_yaml(prompt_name: str) -> Dict[str, Any]:
    """Load an investor slides stage prompt from package YAML."""
...
def combo_params(combo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "bank_symbol": normalize_investor_slide_bank_symbol(bank_symbol),
        "fiscal_year": fiscal_year,
        "quarter": str(combo.get("quarter", "")).strip().upper(),
    }


def normalize_investor_slide_bank_symbol(bank_symbol: Any) -> str:
    """Return the bank symbol exactly as stored in investor slide metadata."""
    ...
    for index, candidate in enumerate(candidates):
        ...
        lines.extend(
            [
                ...
                f"Page title: {candidate.get('name') or 'unknown'} | "
...
def build_source_reference(ref_id: str, chunk: Dict[str, Any]) -> Dict[str, Any]:
    ...
    location_label = str(chunk.get("name") or f"Page {page_number}")
...
def format_evidence_chunks(
    ...
    for chunk in ordered:
        ...
        page_label = chunk.get("name") or f"Page {chunk.get('page_number', 0)}"
        lines.extend(
            [
...
def format_retrieval_response(results: Dict[str, Any]) -> str:
    ...
    for combo_result in results["combo_results"]:
        ...
        if findings:
            ...
            lines.append("- No investor slides content was found for this bank/period.")
        else:
...
def format_finding_references(finding: Dict[str, Any]) -> str:
    ...
    for reference in references:
        rendered.append(f"{reference['link_marker']} | Page: {reference['page']}")
