"""Compatibility adapter for earnings transcript research."""
...
def _metric_from_raw(raw: Dict[str, Any]) -> Optional[MetricObservation]:
    """Build optional metric metadata from a raw transcript finding."""
...
def _table_from_raw(raw: Dict[str, Any]) -> Optional[ResearchTable]:
    """Build optional table metadata from a raw transcript finding."""
    table = raw.get("table")
...
def _evidence_from_reference(reference: Dict[str, Any]) -> EvidenceReference:
    ...
    display = f"Transcripts {location}".strip() if location else "Transcripts"
    return EvidenceReference(
        source_id="transcripts",
        source_label="Transcripts",
        file_id=file_id,
        filename=filename,
        file_type=file_type,
        ...
        href=_source_document_preview_href("transcripts", file_id, file_type, page_number),
        download_href=_source_document_download_href("transcripts", file_id),
        display_label=display,
    )


def _result_from_raw(raw: Dict[str, Any], dropdown_markdown: str) -> ResearchResult:
    """Convert raw transcript pipeline output into the agent result schema."""
    ...
    for combo_result in raw.get("combo_results", []):
        ...
        if not combo_findings:
            gaps.append(
                Gap(
                    combo_label=combo.label,
                    reason="No structured transcript findings were extracted.",
                )
            )
            continue
    ...
    return ResearchResult(
        status=status,
        quick_summary=(
            f"Transcript research produced {len(findings)} finding(s) "
...
async def research_transcripts(
    question: str,
    ...
    """Retrieve and research transcript evidence using the processed-source pipeline."""
