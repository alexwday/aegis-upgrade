"""Staged retrieval pipeline for earnings transcripts."""
...
DATA_TABLE = 'public."aegis-earnings-transcripts-data"'
EMBEDDINGS_TABLE = 'public."aegis-earnings-transcripts-embeddings"'
PROMPTS_DIR = Path(__file__).resolve().parent / "prompts"
TRANSCRIPTS_MODEL_TIER = "small"
...
async def run_retrieval_pipeline(
    ...
    logger.info(
        "subagent.transcripts.pipeline_complete",
...
async def prepare_query(
    ...
    try:
        ...
        logger.warning(
            "subagent.transcripts.query_prep_fallback",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
...
async def embed_prepared_query(prepared: Dict[str, Any], context: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            "subagent.transcripts.query_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        prepared["embeddings"] = {}
...
async def rerank_candidates(
    ...
    try:
        ...
        logger.warning(
            "subagent.transcripts.rerank_keep_all",
            execution_id=context.get("execution_id"),
    ...
    logger.info(
        "subagent.transcripts.rerank_complete",
        execution_id=context.get("execution_id"),
        candidate_count=len(candidates),
...
async def run_research_loop(
    ...
    for iteration_number in range(1, RESEARCH_MAX_ITERATIONS + 1):
        try:
            ...
            logger.warning(
                "subagent.transcripts.research_iteration_failed",
                execution_id=context.get("execution_id"),
...
async def call_research_iteration(
    ...
    replacements = {
        ...
        "source_label": "earnings transcript",
...
async def search_additional_queries(
    ...
    try:
        ...
        logger.warning(
            "subagent.transcripts.additional_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        return []
...
async def call_tool_prompt(
    ...
    response = await complete_with_tools(
        ...
        context=context,
        llm_params={
            "model": getattr(config.llm, TRANSCRIPTS_MODEL_TIER).model,
            "temperature": 0,
...
def load_stage_prompt(prompt_name: str, execution_id: Optional[str] = None) -> Dict[str, Any]:
    """Load a transcript stage prompt from DB, falling back to YAML."""
    logger = get_logger()
    try:
        ...
        logger.warning(
            "subagent.transcripts.prompt_db_fallback",
            execution_id=execution_id,
...
def load_stage_prompt_from_yaml(prompt_name: str) -> Dict[str, Any]:
    """Load a transcript stage prompt from package YAML."""
...
def format_retrieval_response(results: Dict[str, Any]) -> str:
    ...
    for combo_result in results["combo_results"]:
        ...
        if findings:
            ...
            lines.append("- No transcript content was found for this bank/period.")
        else:
