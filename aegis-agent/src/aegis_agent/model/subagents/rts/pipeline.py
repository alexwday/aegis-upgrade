"""Staged retrieval pipeline for RTS."""
...
DATA_TABLE = 'public."aegis-rts-data"'
EMBEDDINGS_TABLE = 'public."aegis-rts-embeddings"'
PROMPTS_DIR = Path(__file__).resolve().parent / "prompts"
RTS_MODEL_TIER = "small"
...
async def run_retrieval_pipeline(
    ...
    logger.info(
        "subagent.rts.pipeline_complete",
...
async def prepare_query(
    ...
    try:
        ...
        logger.warning(
            "subagent.rts.query_prep_fallback",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
...
def fallback_prepared_query(query_text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "hyde_answer": soften_hyde_answer(
            ...
            "management commentary, and any relevant page callouts for the selected banks."
        ),
        "embeddings": {},
        "usage": {},
    }
...
async def embed_prepared_query(prepared: Dict[str, Any], context: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            "subagent.rts.query_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        prepared["embeddings"] = {}
...
async def run_research_loop(
    ...
    for iteration_number in range(1, RESEARCH_MAX_ITERATIONS + 1):
        try:
            ...
            logger.warning(
                "subagent.rts.research_iteration_failed",
                execution_id=context.get("execution_id"),
...
async def search_additional_queries(
    ...
    try:
        ...
        logger.warning(
            "subagent.rts.additional_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        return []
...
def load_stage_prompt(prompt_name: str, execution_id: Optional[str] = None) -> Dict[str, Any]:
    """Load an RTS stage prompt from DB, falling back to YAML."""
    logger = get_logger()
    try:
        ...
        logger.warning(
            "subagent.rts.prompt_db_fallback",
            execution_id=execution_id,
...
def load_stage_prompt_from_yaml(prompt_name: str) -> Dict[str, Any]:
    """Load an RTS stage prompt from package YAML."""
...
def normalize_rts_bank_symbol(bank_symbol: Any) -> str:
    """Return the bank symbol exactly as stored in RTS metadata."""
