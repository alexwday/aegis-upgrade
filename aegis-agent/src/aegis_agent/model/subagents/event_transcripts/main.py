...
async def event_transcripts_agent(
    ...
    """Process event transcript queries.
    ...
    logger.info(
        "subagent.event_transcripts.started",
    ...
    try:
        ...
        results = await retrieve_event_transcripts(
        ...
        logger.error(
            "subagent.event_transcripts.error",
            execution_id=execution_id,
            error=str(exc),
            exc_info=True,
        )
        ...
        yield _chunk(database_id, f"Error retrieving event transcripts: {exc}")


async def retrieve_event_transcripts(
    query_text: str,
    latest_message: str,
    bank_period_combinations: List[Dict[str, Any]],
    context: Dict[str, Any],
    ...
    """Retrieve and research event transcript evidence."""
