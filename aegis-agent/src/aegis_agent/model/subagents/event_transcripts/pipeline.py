"""Staged retrieval pipeline for event transcripts."""
...
DATA_TABLE = 'public."aegis-event-transcripts-data"'
EMBEDDINGS_TABLE = 'public."aegis-event-transcripts-embeddings"'
EVENT_TRANSCRIPTS_MODEL_TIER = "small"
...
PAGE_ID_RE = re.compile(r"page_(\d+)(?:\.(\d+))?", re.IGNORECASE)
...
async def run_retrieval_pipeline(
    ...
    """Run query prep, hybrid search, rerank, transcript-unit fill, and research."""
    logger = get_logger()
    ...
    logger.info(
        "subagent.event_transcripts.pipeline_complete",
...
async def process_combo_retrieval(
    ...
    expanded = await gap_fill_one_page_gaps(reranked, search_semaphore=search_semaphore)
...
async def prepare_query(
    ...
    try:
        ...
        logger.warning(
            "subagent.event_transcripts.query_prep_fallback",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
...
def fallback_prepared_query(query_text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "hyde_answer": soften_hyde_answer(
            "The report should disclose the requested metrics, period comparisons, "
            "management commentary, Q&A exchanges, and selected transcript evidence."
        ),
        "embeddings": {},
        "usage": {},
    }
...
async def embed_prepared_query(prepared: Dict[str, Any], context: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            "subagent.event_transcripts.query_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        prepared["embeddings"] = {}
...
async def rerank_candidates(
    ...
    try:
        ...
        logger.warning(
            "subagent.event_transcripts.rerank_keep_all",
            execution_id=context.get("execution_id"),
    ...
    logger.info(
        "subagent.event_transcripts.rerank_complete",
        execution_id=context.get("execution_id"),
        candidate_count=len(candidates),
...
async def gap_fill_one_page_gaps(
    ...
    """Fill up to two missing transcript units between retrieved chunks from the same file."""
    if not chunks:
        return []
    search_semaphore = search_semaphore or asyncio.Semaphore(MAX_PARALLEL_SEARCH_QUERIES)
    ...
    for file_id, file_chunks in by_file.items():
        ...
        for left, right in zip(page_numbers, page_numbers[1:]):
            missing_count = right - left - 1
            if missing_count <= 0 or missing_count > GAP_FILL_MAX_UNITS:
                continue
            for gap_page in range(left + 1, right):
                gap_spec = (file_id, gap_page)
    ...
    gap_results = await asyncio.gather(
        *[
            load_page_chunks_limited(file_id, gap_page, search_semaphore)
            for file_id, gap_page in gap_specs
        ]
    )
...
async def load_page_chunks_limited(
    file_id: str,
    page_number: int,
    search_semaphore: asyncio.Semaphore,
) -> List[Dict[str, Any]]:
    """Load page chunks behind the shared retrieval concurrency limit."""
    async with search_semaphore:
        return await load_page_chunks(file_id, page_number)


async def load_page_chunks(file_id: str, page_number: int) -> List[Dict[str, Any]]:
    """Load every chunk for one transcript unit number."""
    query = text(
        f"""
        SELECT
            d.source_type,
    ...
    async with get_connection() as conn:
        result = await conn.execute(
            ...
            {
                "file_id": file_id,
                "chunk_prefix": f"page_{page_number}.%",
            },
        )
...
async def run_research_loop(
    ...
    seen_keys = {candidate_key(chunk) for chunk in chunks}
    iterations = []
    ...
    for iteration_number in range(1, RESEARCH_MAX_ITERATIONS + 1):
        try:
            ...
            logger.warning(
                "subagent.event_transcripts.research_iteration_failed",
                execution_id=context.get("execution_id"),
        ...
        chunks = await gap_fill_one_page_gaps(chunks, search_semaphore=search_semaphore)
        seen_keys = {candidate_key(chunk) for chunk in chunks}
...
async def search_additional_queries(
    ...
    try:
        ...
        logger.warning(
            "subagent.event_transcripts.additional_embedding_skipped",
            execution_id=context.get("execution_id"),
            error=str(exc),
        )
        return []
...
async def call_tool_prompt(
    ...
    response = await complete_with_tools(
        ...
        context=context,
        llm_params={
            "model": getattr(config.llm, EVENT_TRANSCRIPTS_MODEL_TIER).model,
            "temperature": 0,
...
def combo_params(combo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "bank_symbol": normalize_transcript_bank_symbol(bank_symbol),
        "fiscal_year": fiscal_year,
        "quarter": str(combo.get("quarter", "")).strip().upper(),
    }


def normalize_transcript_bank_symbol(bank_symbol: Any) -> str:
    """Return the bank symbol exactly as stored in transcript metadata."""
...
def parse_page_number(chunk_id: str) -> Optional[int]:
    """Parse the page number from a page chunk identifier."""
    match = PAGE_ID_RE.search(str(chunk_id))
    if not match:
        return None
    return int(match.group(1))


def parse_page_minor(chunk_id: str) -> int:
    """Parse the within-page chunk number from a page chunk identifier."""
    match = PAGE_ID_RE.search(str(chunk_id))
...
def chunk_sort_key(chunk: Dict[str, Any]) -> Tuple[str, str, int, int, str]:
    """Sort chunks by file, page, sub-chunk, and chunk ID."""
    page_number = parse_page_number(chunk.get("chunk_id", "")) or 0
    return (
        str(chunk.get("file_id", "")),
        str(chunk.get("bank", "")),
        page_number,
        parse_page_minor(chunk.get("chunk_id", "")),
    ...
    for iteration in iterations:
        for finding in iteration.get("findings", []):
            key = (
                ...
                tuple(finding.get("source_ref_ids") or []),
            )
            if key in seen:
                continue
            seen.add(key)
    ...
    for index, candidate in enumerate(candidates):
        ...
        lines.extend(
            [
                ...
                f"{candidate['fiscal_year']} | Transcript unit {candidate['page_number']} | "
                f"Unit title: {candidate.get('name') or 'unknown'} | "
...
def build_source_reference(ref_id: str, chunk: Dict[str, Any]) -> Dict[str, Any]:
    ...
    location_label = str(chunk.get("name") or f"Transcript unit {page_number}")
...
def infer_source_ref_ids(
    ...
    location_matches = [
        ...
        if location
        and (
            location in str(source_catalog[ref_id].get("location") or "").casefold()
...
def format_evidence_chunks(
    ...
    for chunk in ordered:
        ...
        page_label = chunk.get("name") or f"Transcript unit {chunk.get('page_number', 0)}"
        lines.extend(
            [
                f"=== {page_label} (Citation Page {chunk.get('page_number', 0)}) ===",
                (
...
def format_previous_research(iterations: List[Dict[str, Any]]) -> str:
    ...
    for iteration in iterations:
        ...
        for finding in iteration.get("findings", []):
            lines.append(
                f"- {finding['finding']} "
                f"(Transcript unit {finding['page']}, {finding['location_detail']})"
            )
        lines.append("")
...
def format_retrieval_response(results: Dict[str, Any]) -> str:
    ...
    for combo_result in results["combo_results"]:
        ...
        if findings:
            ...
            lines.append("- No event transcript content was found for this bank/period.")
        else:
...
def format_finding_references(finding: Dict[str, Any]) -> str:
    ...
    if not references:
        return f"Transcript unit {finding['page']}, {finding['location_detail']}"
    rendered = []
    for reference in references:
        rendered.append(f"{reference['link_marker']} | Transcript unit: {reference['page']}")
