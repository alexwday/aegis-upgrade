...
async def call_tool_prompt(
    ...
    response = await complete_with_tools(
        ...
        context=context,
        llm_params={
            "model": resolve_research_model(context, EVENT_TRANSCRIPTS_MODEL_TIER),
            "temperature": 0,
...
def format_evidence_chunks(
    ...
    for chunk in ordered:
        ...
        page_label = (
            chunk.get("name") or f"Transcript unit {chunk.get('page_number', 0)}"
        )
        lines.extend(
            [
...
def format_retrieval_response(results: Dict[str, Any]) -> str:
    ...
    for combo_result in results["combo_results"]:
        ...
        if findings:
            ...
            lines.append(
                "- No event transcript content was found for this bank/period."
            )
        else:
...
def format_finding_references(finding: Dict[str, Any]) -> str:
    ...
    for reference in references:
        rendered.append(
            f"{reference['link_marker']} | Transcript unit: {reference['page']}"
        )
