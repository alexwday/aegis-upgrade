"""Configuration helpers for source pipeline modules."""
...
MODULE_ROOT = SOURCE_CONTEXT.root
REPO_ROOT = SOURCE_CONTEXT.root
ENV_PATH = SOURCE_CONTEXT.env_path
DEFAULT_LOCAL_INPUT_PATH = SOURCE_CONTEXT.default_input_path
DEFAULT_LOCAL_OUTPUT_PATH = SOURCE_CONTEXT.default_output_path
DEFAULT_DATA_SOURCE = SOURCE_CONTEXT.data_source
DEFAULT_MASTER_DATA_TABLE_NAME = SOURCE_CONTEXT.master_table
TOKENIZER_CACHE_PATH = SOURCE_CONTEXT.tokenizer_cache_path
...
RAW_OCR_IMAGE_DETAILS = {"auto", "high", "low", "original"}
...
if SOURCE_CONTEXT.has_extraction_model_alias:
    STAGE_MODEL_ALIASES["extraction"] = "large"
...
class EnrichmentBudgetConfig:
    """Token budgets used by the enrichment stage."""
...
class EnrichmentConfig:
    ...
    ``max_workbook_workers`` bounds concurrent document pipelines,
    ``max_sheet_workers`` bounds in-document batch fan-out, and
...
class PdfExtractionConfig:
    """Runtime controls for PDF vision extraction.

    The raw OCR settings drive the single chat-completions vision request made
    for each rendered page. Region and crop settings are retained as legacy
    artifact compatibility knobs, but the current extraction path does not
    issue crop correction calls.
    """

    vision_dpi_scale: float = 2.0
    page_workers: int = 4
    raw_ocr_model: str = SOURCE_CONTEXT.pdf_raw_ocr_model
    raw_ocr_detail: str = "high"
    raw_ocr_reasoning_effort: str | None = "low"
    raw_ocr_max_output_tokens: int = 12000
    region_workers: int = 3
    max_retries: int = 3
    retry_delay_seconds: float = 2.0
    crop_padding: float = 0.025
    boundary_retry_enabled: bool = True
    boundary_retry_padding: float = 0.075
    boundary_retry_external_label_padding: float = 0.14
    boundary_retry_max_attempts: int = 1
    progress_log_interval_seconds: float = 30.0


def load_config(env_path: Path | None = None, override: bool = False) -> None:
    """Load the active source .env file into process environment.

    Existing process environment values win by default. This lets a global
    OPENAI_API_KEY remain active even when the source .env contains a blank value.
    """
...
def _optional_choice(name: str, allowed: set[str], default: str = "") -> str | None:
    """Return a lower-cased choice, or None when explicitly blank."""
    value = _env(name, default).lower()
    if not value:
        return None
    if value not in allowed:
        allowed_values = ", ".join(sorted(allowed))
        raise ValueError(
            f"{name} must be blank or one of: {allowed_values}; got {value!r}"
        )
    return value
...
def get_oauth_config() -> dict[str, str]:
    """Return OAuth config using the env names in .env."""
    config = OAuthConfig(
...
def get_pdf_extraction_config() -> PdfExtractionConfig:
    """Return PDF extraction settings from environment values."""
    return PdfExtractionConfig(
        vision_dpi_scale=_float("VISION_DPI_SCALE", "2.0", 0.1, required=True)
        or 2.0,
        page_workers=_int("EXTRACTION_PAGE_WORKERS", "4", 1),
        raw_ocr_model=_env("PDF_RAW_OCR_MODEL", SOURCE_CONTEXT.pdf_raw_ocr_model),
        raw_ocr_detail=_choice(
            "PDF_RAW_OCR_DETAIL",
            RAW_OCR_IMAGE_DETAILS,
            "high",
        ),
        raw_ocr_reasoning_effort=_optional_choice(
            "PDF_RAW_OCR_REASONING_EFFORT",
            REASONING_EFFORTS,
            "low",
        ),
        raw_ocr_max_output_tokens=_int("PDF_RAW_OCR_MAX_OUTPUT_TOKENS", "12000", 1),
        region_workers=_int("EXTRACTION_REGION_WORKERS", "3", 1),
        max_retries=_int("PDF_VISION_MAX_RETRIES", "3", 1),
        ...
        or 0.0,
        crop_padding=_float("PDF_CROP_PADDING", "0.025", 0.0, required=True)
        or 0.0,
        boundary_retry_enabled=_bool("PDF_BOUNDARY_RETRY_ENABLED", True),
        boundary_retry_padding=_float(
        ...
        or 0.0,
        boundary_retry_external_label_padding=_float(
        ...
        or 0.0,
        boundary_retry_max_attempts=_int("PDF_BOUNDARY_RETRY_MAX_ATTEMPTS", "1", 0),
        progress_log_interval_seconds=_float(
...
def get_master_data_table_name() -> str:
    ...
    table_name = (
        configured_name if configured_name else DEFAULT_MASTER_DATA_TABLE_NAME
    )
...
def get_base_path() -> Path | str:
    ...
    aegis-documents folder for the active source. For NAS input,
    INPUT_BASE_PATH is the optional path prefix inside the share.
    """
...
def get_output_base_path() -> Path | str:
    ...
    For local output, a blank OUTPUT_BASE_PATH defaults to the active source's
    data-output/master folder. The directory is not required to exist because
    callers may create it when writing artifacts. For NAS output,
    OUTPUT_BASE_PATH is the optional path prefix inside the configured share.
    """
