"""Create the public PostgreSQL tables for Pillar 3 retrieval.
...
* ``public."aegis-pillar3-data"`` for chunk-level source rows.
* ``public."aegis-pillar3-embeddings"`` for long-form vector rows.
...
DEFAULT_DATA_TABLE_NAME = "aegis-pillar3-data"
DEFAULT_EMBEDDINGS_TABLE_NAME = "aegis-pillar3-embeddings"
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg2.connect(
        **db_config,
        application_name="aegis-pillar3-create-master-tables",
    ) as conn:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create public Pillar 3 retrieval tables.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
