...
class EnrichedContentUnit:
    """One transcript content unit before embeddings."""
...
def _format_content_batch(
    ...
    for unit in batch:
        ...
        attrs = [
            ...
            f'item_type="{_attribute_token(item_label)}"',
...
def _build_file_metadata(document: EnrichmentDocument) -> str:
    ...
    item_types = sorted(
        {_attribute_token(_unit_item_label(unit)) for unit in document.content_units}
    )
    primary_item_label = _document_item_label(document)
    lines = [
        f"filename: {document.filename}",
        f"filetype: {document.filetype}",
        f"primary_item_type: {_attribute_token(primary_item_label)}",
...
def _attribute_token(value: str) -> str:
    """Return a prompt-safe token for an item label."""
    return value.strip().lower().replace(" ", "_") or "item"


def _is_transcript_source(source: dict[str, Any]) -> bool:
    """Return whether a source record is an earnings transcript."""
    value = str(
        source.get("data_source")
        or source.get("source_type")
        or ""
    )
    return value.strip().replace("_", "-").lower() == "earnings-transcripts"
...
def _source_title(document: EnrichmentDocument) -> str:
    ...
    if data_source == "earnings-transcripts":
