"""Create the public PostgreSQL tables for earnings transcript retrieval.
...
* ``public."aegis-earnings-transcripts-data"`` for chunk-level source rows.
* ``public."aegis-earnings-transcripts-embeddings"`` for long-form vector rows.
...
DEFAULT_DATA_TABLE_NAME = "aegis-earnings-transcripts-data"
DEFAULT_EMBEDDINGS_TABLE_NAME = "aegis-earnings-transcripts-embeddings"
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg2.connect(
        **db_config,
        application_name="aegis-earnings-transcripts-create-master-tables",
    ) as conn:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create public earnings transcript retrieval tables.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
