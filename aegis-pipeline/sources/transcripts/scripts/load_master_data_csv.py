...
``database.pipeline.finalize``. On ``--apply`` it stages both CSVs in temporary
tables, truncates the public targets, and inserts staged rows in one transaction.

Examples:
    venv/bin/python scripts/load_master_data_csv.py
    venv/bin/python scripts/load_master_data_csv.py --apply
    venv/bin/python scripts/load_master_data_csv.py --env-file /path/.env --apply
"""
...
from scripts.create_master_data_table import (  # noqa: E402
    DEFAULT_DATA_TABLE_NAME,
    DEFAULT_EMBEDDINGS_TABLE_NAME,
    PUBLIC_SCHEMA,
)
...
DATA_STAGING_TABLE_NAME = "aegis_earnings_transcripts_data_load_stage"
EMBEDDINGS_STAGING_TABLE_NAME = "aegis_earnings_transcripts_embeddings_load_stage"
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg2.connect(
        **db_config,
        application_name="aegis-earnings-transcripts-load-master-csvs",
    ) as conn:
        loaded_data_rows = _refresh_table(
            conn,
            table_name=config.data_table,
            staging_table_name=DATA_STAGING_TABLE_NAME,
        ...
        loaded_embedding_rows = _refresh_table(
            ...
            staging_table_name=EMBEDDINGS_STAGING_TABLE_NAME,
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Refresh public earnings transcript retrieval tables.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
...
def _resolve_config(args: argparse.Namespace) -> LoadConfig:
