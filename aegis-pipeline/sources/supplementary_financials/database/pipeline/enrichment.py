"""Enrich sheet-level XLSX chunks with metadata, keywords, and summaries.

This stage mirrors Aegis Retriever Stage 5 for the XLSX path. It loads the
chunking artifacts, treats every sheet as one content unit and one section,
then performs the enrichment call sequence: document metadata, content
keyword/metric extraction, and section summaries.
"""
...
VALID_STRUCTURE_TYPES = frozenset(
    {"sheet_based", "chapters", "sections", "topic_based", "semantic"}
)
...
class EnrichedContentUnit:
    """One sheet-level content unit before embeddings."""
...
class EnrichmentDocument:
    """One workbook flowing through enrichment."""
...
class _ProcessedWorkbook:
    """Result returned after one workbook has completed enrichment."""
...
class _WorkbookProgress:
    """Thread-safe tracker state for one workbook's enrichment progress."""
...
class _EnrichmentProgressTracker:
    """Emit compact aggregate enrichment progress for concurrent workbooks."""
    ...
    def start(self, index: int, record: ManifestRecord) -> None:
        """Track a workbook as active and log the start once."""
    ...
    def update(
        ...
        *,
        ...
        """Update one workbook and maybe log an aggregate progress snapshot."""
        with self._lock:
    ...
    def complete(self, file_id: str, duration_seconds: float) -> None:
        """Mark a workbook completed and log one final per-file line."""
        with self._lock:
            state = self._states.get(file_id)
            if state is None:
                return
        ...
        self._logger.info(
            "Finished workbook %d/%d: %s sheets=%d duration=%.1fs",
            completed,
            self._total_files,
            state.file_name,
            state.sheet_total,
    ...
    def _completed_count_locked(self) -> int:
        """Return completed workbook count while the caller holds the lock."""
        return sum(1 for state in self._states.values() if state.completed)


def _render_progress_state(state: _WorkbookProgress) -> str:
    """Render per-workbook sheet counts in one compact progress segment."""
...
def run_enrichment_stage(
    ...
    """Run XLSX sheet enrichment over existing chunking artifacts.
    ...
    Args:
        ...
        config: Optional enrichment runtime config for tests or overrides,
            including workbook concurrency, LLM call concurrency, retry policy,
        ...
        max_content_units_per_file: Optional sheet/content-unit limit per file.
        max_workbook_workers: Optional workbook concurrency override. When not
    ...
    logger.info(
        "Enrichment queued: files=%d workbook_workers=%d "
        "sheet_workers=%d llm_workers=%d progress_interval=%.1fs",
        len(records),
        workbook_workers,
...
def _process_enrichment_workbook(
    ...
    """Run all enrichment phases for one workbook and write its artifacts."""
...
def enrich_document_metadata(
    ...
    prompt = load_prompt("doc_metadata", PROMPTS_DIR)
...
def detect_sheet_sections(document: EnrichmentDocument) -> None:
    """Create one section per XLSX sheet and assign units to sections."""
    sections = []
    ...
    for sequence, item_number in enumerate(sorted(grouped), start=1):
        ...
        title = units[0].item_title or f"Sheet {item_number}"
...
def extract_content_metadata(
    ...
    The optional callback receives completed sheet count and total sheet count
    after each batch finishes.
    """
    prompt = load_prompt("content_extraction", PROMPTS_DIR)
...
def summarize_sections(
    ...
    """Populate sheet-section summaries, keywords, and metrics.

    Section batches fan out up to ``config.max_sheet_workers``. The optional
    callback receives completed sheet-section count and total sheet-section
    count after each batch finishes.
    """
    prompt = load_prompt("section_summary", PROMPTS_DIR)
    ...
    if not batches:
        ...
        # batch gets the same stable sheet outline instead of a progressive TOC.
...
def write_enrichment_artifacts(
    ...
    """Write inspectable enrichment artifacts for one workbook."""
...
def _format_content_batch(
    ...
    """Build keyword/metric extraction prompt input using full sheet content."""
    source_context = _source_context(document)
    context_lines = []
...
def _format_section_summary_batch(
    ...
    """Build section-summary prompt input using full sheet content."""
...
def _build_file_metadata(document: EnrichmentDocument) -> str:
    ...
    lines = [
        ...
        f"total_pages: {len({unit.item_number for unit in document.content_units})}",
    ]
...
def _build_page_names(document: EnrichmentDocument) -> str:
    """Build sheet names prompt block."""
    seen_items = set()
    lines = []
    for unit in sorted(document.content_units, key=lambda item: item.item_number):
        ...
        lines.append(f"Sheet {unit.item_number}: {unit.item_title}")
    return "\n".join(lines)
...
def _build_metadata_content(document: EnrichmentDocument, budget: int) -> str:
    """Collect raw sheet content up to the configured metadata budget."""
    parts = []
    used = 0
    for unit in sorted(
        document.content_units,
...
def _load_enrichment_document(
    ...
    """Build one enrichment document from one workbook's chunking artifacts."""
...
def _resolve_workbook_workers(requested_workers: int, record_count: int) -> int:
    """Return the bounded workbook worker count for this enrichment run."""
    if record_count <= 0:
        return 0
...
def _parallel_worker_count(config: EnrichmentConfig, item_count: int) -> int:
    """Return worker count for independent in-workbook enrichment batches."""
    if item_count <= 0:
        return 0
...
def _sheet_count(document: EnrichmentDocument) -> int:
    """Return distinct source-sheet count represented by content units."""
    return len({unit.item_number for unit in document.content_units})


def _section_count(document: EnrichmentDocument) -> int:
    """Return sheet-section count represented by the enrichment document."""
...
def _unit_ids_by_sheet(
    ...
    """Return content-unit ids grouped by source sheet number."""
...
def _completed_sheet_count(
    ...
    """Return sheets whose content units have all finished enrichment."""
    return sum(
        1
...
def _group_units_by_item(
    ...
    """Return content units grouped by source sheet number."""
...
def _source_title(document: EnrichmentDocument) -> str:
    ...
    return f"{bank.upper()} {year} {quarter.upper()} Financial Supplement"
...
def _enrichment_manifest_record(
    ...
    """Build a per-workbook enrichment manifest record."""
    return {
        "stage": "enrichment",
        "processed_at": processed_at,
        "duration_seconds": round(duration_seconds, 6),
