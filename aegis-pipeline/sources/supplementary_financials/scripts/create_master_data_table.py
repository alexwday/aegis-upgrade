"""Create the public PostgreSQL tables for financial supplement retrieval.
...
* ``public."aegis-financial-supp-data"`` for chunk-level source rows.
* ``public."aegis-financial-supp-embeddings"`` for long-form vector rows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg2.connect(
        **db_config,
        application_name="aegis-financial-supp-create-master-tables",
    ) as conn:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create public financial supplement retrieval tables.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
