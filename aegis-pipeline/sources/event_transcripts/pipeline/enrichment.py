...
def _is_transcript_source(source: dict[str, Any]) -> bool:
    """Return whether a source record is an event transcript."""
    value = str(
        source.get("data_source")
        or source.get("source_type")
        or ""
    )
    return value.strip().replace("_", "-").lower() == "event-transcripts"
...
def _source_title(document: EnrichmentDocument) -> str:
    ...
    if data_source == "event-transcripts":
        source_name = "Event Transcripts"
