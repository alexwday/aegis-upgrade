"""Single-pass PDF OCR extraction stage for investor-slide decks."""
...
def run_extraction_stage(
    ...
    """Extract all pending investor-slide PDFs selected by the manifest stage.

    Args:
        input_base_path: Optional local investor-slides input folder override.
...
def extract_pdf_document_pages(
    ...
    slide_ocr_prompt = load_prompt("slide_ocr", PROMPTS_DIR)
    with open_rendered_pdf(
        ...
        context = PdfExtractionContext(
            ...
            slide_ocr_prompt=slide_ocr_prompt,
        )
...
def _extract_single_page(
    ...
    """OCR one rendered page with a single full-page GPT-5.4 request."""
    page_context = (
        f"{context.file_label} page {rendered_page.page_number}/"
        f"{context.total_pages} raw ocr"
    )
    markdown = _call_raw_slide_ocr_with_retries(
        context,
    ...
    layout = PageLayout(
        ...
        rationale="Single-pass GPT-5.4 full-slide OCR.",
    )
...
def _call_raw_slide_ocr_with_retries(
    context: PdfExtractionContext,
    img_bytes: bytes,
    page_context: str,
) -> str:
    """Call GPT-5.4 raw OCR with retry handling and return markdown."""
...
def write_pdf_document_artifacts(
    ...
    *,
    ...
    payload = {
        "stage": "extraction",
        "extraction_type": "pdf_investor_slides_raw_ocr",
        "processed_at": processed_at,
...
def _resolve_input_base_path(input_base_path: Path | None) -> Path:
    """Resolve the local investor-slides input folder."""
