"""Create the public PostgreSQL tables for investor slide retrieval.
...
* ``public."aegis-investor-slides-data"`` for chunk-level source rows.
* ``public."aegis-investor-slides-embeddings"`` for long-form vector rows.
...
DEFAULT_DATA_TABLE_NAME = "aegis-investor-slides-data"
DEFAULT_EMBEDDINGS_TABLE_NAME = "aegis-investor-slides-embeddings"
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg2.connect(
        **db_config,
        application_name="aegis-investor-slides-create-master-tables",
    ) as conn:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create public investor slide retrieval tables.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
