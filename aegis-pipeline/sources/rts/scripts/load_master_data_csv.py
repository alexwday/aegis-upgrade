...
DATA_STAGING_TABLE_NAME = "aegis_rts_data_load_stage"
EMBEDDINGS_STAGING_TABLE_NAME = "aegis_rts_embeddings_load_stage"
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg2.connect(
        **db_config,
        application_name="aegis-rts-load-master-csvs",
    ) as conn:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Refresh public RTS retrieval tables.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
