...
function App() {
  ...
  const [artifacts, setArtifacts] = React.useState<Artifact[]>(exampleArtifacts);
  ...
  const [researchDepth, setResearchDepth] = React.useState<ResearchDepth>("short");
  ...
  const loadAvailabilityCatalog = React.useCallback(async () => {
    const response = await fetch("/api/v2/availability?limit=2000");
    if (!response.ok) throw new Error(await response.text());
  ...
  function handleEvent(event: V2Event) {
    ...
    if (event.type === "chat.message") {
      const payload = event.payload as { role?: ChatMessage["role"]; content?: string };
            ...
            <div className="composer-input-shell">
              ...
              <div className="composer-input-main">
                <div className="composer-context-row">
                  ...
                  <ResearchModeControl value={researchDepth} setValue={setResearchDepth} />
                </div>
                <textarea
                  ref={inputRef}
                  value={input}
...
function QueryFilterPanel({
  ...
  const sourceIds = SOURCE_OPTIONS.map(([sourceId]) => sourceId);
  ...
  return (
    <div className="query-filter-popover">
      ...
      <div className="query-filter-list">
        {SOURCE_OPTIONS.map(([sourceId, label]) => (
          <button
            key={sourceId}
            type="button"
            className={`query-filter-row ${filters.source_ids.includes(sourceId) ? "selected" : ""}`}
            aria-pressed={filters.source_ids.includes(sourceId)}
            onClick={() => toggleSourceFilter(sourceId)}
          >
            <SourceIcon sourceId={sourceId} size={15} />
            <span>{label}</span>
          </button>
        ))}
      </div>
...
function ResearchModeControl({
  ...
  value: ResearchDepth;
  setValue: (value: ResearchDepth) => void;
}) {
  return (
